...
const PostitWrapper = ({ data }) => {
    ...
    const onDragEnd = ({ source, destination }) => {
        ...
        const destPrevId = items[destKey][destIdx - 1]
            ? items[destKey][destIdx - 1].commentId
            : null;

        const destNextId = items[destKey][destIdx] ? items[destKey][destIdx].commentId : -1;
        ...
        if (sourcePrevId !== null) {
            ...
            updatePayload.push({ commentId: sourcePrevId, after: sourceNextId });
        }
        ...
        if (destPrevId !== null) {
            ...
            updatePayload.push({ comentId: destPrevId, after: moveId });
        }

        // move의 after update
        items[sourceKey][sourceIdx].after = destNextId;
        updatePayload.push({ commentId: moveId, after: destNextId });
        ...
        setComments(items);
        mutate(
            { updatePositionList: updatePayload },
            {
